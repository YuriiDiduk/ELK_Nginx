...
resource "null_resource" "move_logstash_file" {
  ...
  provisioner "file" {
    content = templatefile("./templates/logstash_config.tpl",{
    elasticsearch = aws_instance.elastic_nodes[0].public_ip
  })
...
###########################################################################nginx
/*
resource "null_resource" "move_nginx_file" {
  depends_on = [ 
    aws_instance.logstash
   ]
  connection {
...
output "logstash_ip_addr" {
  value = join(":",[aws_instance.logstash.public_ip,"80"])
}
